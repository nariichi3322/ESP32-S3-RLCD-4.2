...
这是一个基于 **ESP32-S3** 和 **4.2 英寸 RLCD 屏幕** 的低功耗本地时钟固件。当前版本只开放原离线模式的本地界面；Wi-Fi 仅用于设置、NTP 校时与用户主动 OTA。

## 当前版本：本地界面 + NTP + 手动 OTA

- 首次开机若没有 Wi-Fi 配置，设备会开启设置 AP；连接后访问 `http://192.168.4.1/`。
- 设置页只保存主/备用 Wi-Fi 与 NTP 服务器。默认服务器为 `pool.ntp.org`，也可填写其它主机名。
- 配置完成后只显示温湿时钟、日历与温湿历史等原离线模式页面。
- 设备在开机、手动校时及上次成功校时后每 24 小时同步一次；其余时间关闭 Wi-Fi。
- 菜单中的“设置模式”可随时重新开启 AP，修改 Wi-Fi 或 NTP 服务器。
- 菜单中的“系统 → 检查更新”可手动检查 OTA；发现新版本后需在 60 秒内再次按 BOOT 确认安装。
- 天气、每日文字、小智 AI 与网络检测不再提供用户入口，也不会触发日常联网；OTA 也不会自动检查。
- 本分支的 OTA 清单由 `nariichi3322` 分支发布，`main` 仅用于跟随上游。

下方旧版完整功能说明仅保留作为原项目架构参考；当前固件行为以上述说明为准。
