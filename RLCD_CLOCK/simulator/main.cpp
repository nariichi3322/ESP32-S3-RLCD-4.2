...
#ifndef WEATHER_CLOCK_PROJECT_VER
#define WEATHER_CLOCK_PROJECT_VER "unknown"
#endif
static const char *APP_VERSION = WEATHER_CLOCK_PROJECT_VER;
#endif
...
static void build_aggregate_clock_preview_ui()
{
    ...
    aggregate_clock_set_text(view.icon,weather_icon_text(WeatherIconKind::kCloudy).c_str());
    ...
    if(theme && (strcmp(theme,"day")==0 || strcmp(theme,"rain")==0 || strcmp(theme,"snow")==0)) {
        ...
        aggregate_clock_set_text(
            view.icon,
            weather_icon_text(snow ? WeatherIconKind::kSnow
                                   : rain ? WeatherIconKind::kRain
                                          : WeatherIconKind::kClear).c_str());
...
static sdl_preview_mode::Selection build_selected_preview(const char *preview_mode)
{
    ...
    if (selection.history) {
        ...
        make_black_bar(screen,
                       ...
                       ui_work_page_layout::kTopSeparatorHeight);
        g_work_page_day_progress.build_day(screen, local, 59);
        build_aggregate_clock_preview_body(screen);
        g_clock.update_time(local);
    } else {
...
static void render_web_demo()
{
    ...
    if(g_demo.scene==WebDemoState::Settings || g_demo.scene==WebDemoState::Pages || g_demo.scene==WebDemoState::Order) {
        ...
        if(g_demo.scene==WebDemoState::Info) {
            ...
            const char *items[]={"本地网络","公网地址","定位服务","DNS","Open-Meteo","时间同步","每日文字","公网连接","OTA清单"};
