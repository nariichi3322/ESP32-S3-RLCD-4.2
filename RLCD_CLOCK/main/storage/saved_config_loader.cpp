...
namespace {
constexpr WorkPageMask kPageMaskV4KnownBits = network_page_storage::kLegacyV4KnownPageMask;
constexpr WorkPageMask kPageMaskV6KnownBits = network_page_storage::kLegacyV6KnownPageMask;
constexpr WorkPageMask kCurrentPageMask = network_page_storage::kCurrentKnownPageMask;
...
static_assert((kPageMaskV6KnownBits & work_page_mask_bit(kWorkPageAggregateClock)) != 0,
              "main V6 page mask must include aggregate clock bit");
static_assert(kCurrentPageMask == static_cast<WorkPageMask>((1U << kWorkPageCount) - 1U),
              "current page mask must cover every work page");
...
bool apply_loaded_page_config(WorkPageMask page_mask,
                              const uint8_t *page_order,
                              bool have_page_order)
{
    const WorkPageMask saved_mask = normalize_work_page_enabled_mask(page_mask);
...
void initialize_loaded_saved_config(LoadedSavedConfig *loaded)
{
    ...
    loaded->page_mask = kCurrentPageMask;
