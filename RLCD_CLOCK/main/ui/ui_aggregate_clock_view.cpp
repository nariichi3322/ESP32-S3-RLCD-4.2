...
lv_obj_t *label(lv_obj_t *root,int x,int y,int w,int h,const char *text,
    ...
    lv_obj_set_pos(p,x,y); lv_obj_set_size(p,w,h);
    lv_obj_set_style_text_font(p,ui_font_for_text(text, font ? font : ui_font(UiFontRole::Body16)),0);
...
void aggregate_clock_view_build(lv_obj_t *root,AggregateClockView &v,lv_color_t *const buffers[3]) {
    ...
    v.range=label(root,26,274,208,17,ui_text(UiTextId::UiPlaceholder));
    v.day=label(root,251,176,74,56,ui_text(UiTextId::UiPlaceholder),&lv_font_montserrat_48);
    lv_obj_set_style_text_align(v.day,LV_TEXT_ALIGN_CENTER,0);
    v.month=label(root,325,178,54,26,ui_text(UiTextId::AggregateMonthPlaceholder),
                 ui_font(UiFontRole::Calendar22));
    v.lunar=label(root,325,204,54,27,ui_text(UiTextId::AggregateLunarDayPlaceholder),
                  ui_font(UiFontRole::Calendar22));
