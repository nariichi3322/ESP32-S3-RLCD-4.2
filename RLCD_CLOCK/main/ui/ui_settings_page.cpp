    ...
    int page)
{
    ...
    return (snapshot.work_page_enabled_mask &
            work_page_mask_bit(page)) != 0;
}

int collect_visible_work_page_order(int *indices,
                                    uint8_t *pages,
                                    size_t capacity,
                                    WorkPageMask enabled_mask)
{
    ...
    for (int order_index = 0; order_index < kWorkPageCount && (size_t)count < capacity; ++order_index) {
        if ((enabled_mask & work_page_mask_bit(order[order_index])) != 0) {
    ...
    char secondary_items[][kSettingsSecondaryTextSize])
{
    ...
    if (navigation.page_order_mode || navigation.page_toggle_mode) {
        ...
        if (index + manager_offset >= manager_item_count) {
        ...
        if (navigation.page_order_mode) {
            format_secondary_text(secondary_items,
                                  ...
                                  index + manager_offset + 1,
                                  work_page_name(visible_order_pages[index + manager_offset]));
            hide_settings_switch_slot(index);
        } else {
            set_secondary_text(secondary_items,
                               index,
                               work_page_name(index + manager_offset));
            if (s_settings_switch_dots[index]) {
                                 ...
                                 const SettingsSecondaryStateSnapshot &secondary_state)
{
    ...
    if (visible && primary == kSettingsPrimarySound) {
        ...
        dot_visible = true;
         dot_on = work_page_enabled_in_switch_snapshot(secondary_state, work_page_id);
    } else if (visible &&
               primary == kSettingsPrimaryDisplay &&
               !navigation.page_toggle_mode &&
    ...
    char secondary_items[][kSettingsSecondaryTextSize])
{
    ...
    int visible_order_count = navigation.page_order_mode
                                  ...
                                  : 0;
    int manager_offset = 0;
    if (navigation.page_order_mode && visible_order_count > 0) {
        int selected_visible = 0;
        for (int i = 0; i < visible_order_count; ++i) {
            if (visible_order_indices[i] == navigation.page_order_selection) {
                selected_visible = i;
                break;
            }
        }
        manager_offset = (selected_visible / settings_layout::kSettingsGridCapacity) *
                         settings_layout::kSettingsGridCapacity;
    } else if (navigation.page_toggle_mode) {
        manager_offset = (selected / settings_layout::kSettingsGridCapacity) *
                         settings_layout::kSettingsGridCapacity;
    }
    for (int i = 0; i < kSettingsSecondaryMaxCount; ++i) {
        int slot = kSettingsPrimaryCount + i;
        ...
        if (navigation.page_order_mode) {
            visible = i + manager_offset < visible_order_count;
        } else if (navigation.page_toggle_mode) {
            visible = i + manager_offset < kWorkPageCount;
        }
        ...
        if (visible) {
            ...
            if (selection_changed) {
                int order_index = navigation.page_order_mode
                                      ? visible_order_indices[i + manager_offset]
                                      : -1;
                bool selected_item = navigation.page_order_mode
                                         ? order_index == navigation.page_order_selection
                                     : navigation.page_toggle_mode
                                         ? i + manager_offset == selected
        ...
        update_settings_switch_slot(
            i,
            navigation.page_toggle_mode ? i + manager_offset : i,
            primary,
            selected - manager_offset,
            visible,
            navigation,
            secondary_state);
    }
...
bool update_settings_page()
{
    ...
    if (s_settings_system_page_label) {
        const bool display_manager = primary == kSettingsPrimaryDisplay &&
                                     (navigation.page_toggle_mode ||
                                      navigation.page_order_mode);
        int manager_page = 0;
        if (display_manager) {
            if (navigation.page_toggle_mode) {
                manager_page = selected / settings_layout::kSettingsGridCapacity;
            } else if (visible_order_count > 0) {
                int selected_visible = 0;
                for (int i = 0; i < visible_order_count; ++i) {
                    if (visible_order_indices[i] == navigation.page_order_selection) {
                        selected_visible = i;
                ...
                manager_page = selected_visible /
                               settings_layout::kSettingsGridCapacity;
            }
        }
        const bool indicator_visible =
            (primary == kSettingsPrimarySystem || display_manager) &&
            !ota_detail_panel_visible;
        lv_obj_set_pos(s_settings_system_page_label,
                       settings_layout::kSettingsSystemPageIndicatorX,
                       display_manager
                           ? settings_layout::kSettingsDisplayPageIndicatorY
                           : settings_layout::kSettingsSystemPageIndicatorY);
