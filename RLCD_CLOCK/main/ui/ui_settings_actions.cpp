...
constexpr WorkPageMask kAllWorkPageMask =
    static_cast<WorkPageMask>((1U << kWorkPageCount) - 1);
...
WorkPageMask toggled_work_page_mask(WorkPageMask current_mask, int page)
{
    if (!is_valid_work_page_id(page)) {
        return static_cast<WorkPageMask>(current_mask & kAllWorkPageMask);
    }
    WorkPageMask page_mask = work_page_mask_bit(page);
    return static_cast<WorkPageMask>((current_mask ^ page_mask) & kAllWorkPageMask);
}
...
static_assert(kWorkPageCount > 0 &&
                  kWorkPageCount <= static_cast<int>(sizeof(WorkPageMask) * 8),
              "work page mask in settings UI must represent every page");
    ...
    SettingsNavigationSnapshot navigation)
{
    if (navigation.page_toggle_mode) {
        ...
        const bool page_was_enabled =
            (previous_mask & work_page_mask_bit(page)) != 0;
