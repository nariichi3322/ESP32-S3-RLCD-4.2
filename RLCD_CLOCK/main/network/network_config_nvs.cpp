...
esp_err_t set_nvs_u16_if_ok(nvs_handle_t nvs,
                            esp_err_t err,
                            const char *key,
                            uint16_t value)
{
...
bool nvs_u16_matches(nvs_handle_t nvs, const char *key, uint16_t expected)
{
    if (!key) {
        return false;
    }
    uint16_t value = 0xffff;
    return nvs_get_u16(nvs, key, &value) == ESP_OK && value == expected;
}

esp_err_t write_changed_nvs_u16(nvs_handle_t nvs,
                                esp_err_t err,
                                const char *key,
                                uint16_t value,
                                bool *changed)
{
    ...
    if (nvs_u16_matches(nvs, key, value)) {
        return ESP_OK;
    }
    esp_err_t write_err = set_nvs_u16_if_ok(nvs, err, key, value);
    if (write_err == ESP_OK && changed) {
        *changed = true;
    }
    return write_err;
