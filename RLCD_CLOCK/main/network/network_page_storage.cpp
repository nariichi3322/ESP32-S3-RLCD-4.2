...
{
    uint16_t v7_mask = 0;
    if (nvs_get_u16(nvs, kPageMaskV7Key, &v7_mask) == ESP_OK &&
        (static_cast<WorkPageMask>(v7_mask) & ~kCurrentKnownPageMask) == 0) {
        return normalize_work_page_enabled_mask(static_cast<WorkPageMask>(v7_mask));
    }
    uint8_t page_mask = 0;
    if (nvs_get_u8(nvs, kPageMaskV6Key, &page_mask) == ESP_OK &&
        (static_cast<WorkPageMask>(page_mask) & ~kLegacyV6KnownPageMask) == 0) {
        return normalize_work_page_enabled_mask(migrate_v6_page_mask(page_mask));
    }
    if (nvs_get_u8(nvs, kPageMaskV5Key, &page_mask) == ESP_OK &&
        (static_cast<WorkPageMask>(page_mask) & ~kLegacyV5KnownPageMask) == 0) {
        return normalize_work_page_enabled_mask(migrate_v5_page_mask(page_mask));
    }
    if (nvs_get_u8(nvs, kPageMaskV4Key, &page_mask) == ESP_OK &&
        (static_cast<WorkPageMask>(page_mask) & ~kLegacyV4KnownPageMask) == 0) {
        return normalize_work_page_enabled_mask(migrate_v4_page_mask(page_mask));
    }
    return kCurrentKnownPageMask;
}
...
bool read_saved_page_order(nvs_handle_t nvs, uint8_t *page_order, size_t page_order_size)
{
    ...
    if (nvs_get_blob(nvs, kPageOrderV7Key, page_order, &stored_len) == ESP_OK &&
        ...
        work_page_order_policy::order_is_valid(page_order, page_order_size)) {
        return true;
    }
    uint8_t v6_order[kLegacyV6WorkPageCount] = {};
    stored_len = sizeof(v6_order);
    if (nvs_get_blob(nvs, kPageOrderV6Key, v6_order, &stored_len) == ESP_OK &&
        stored_len == sizeof(v6_order) &&
        migrate_v6_page_order(v6_order, sizeof(v6_order), page_order, page_order_size)) {
        return true;
    }
