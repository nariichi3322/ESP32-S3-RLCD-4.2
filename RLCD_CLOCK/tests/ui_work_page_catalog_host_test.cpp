...
constexpr WorkPageMask page_bit(int page)
{
...
void expect_order_policy_boundaries()
{
    ...
    const WorkPageMask enabled = page_bit(kWorkPageXiaozhiAI) |
...
int main()
{
    ...
    work_page_enabled_mask_store(static_cast<WorkPageMask>((1U << kWorkPageCount) - 1U));
    ...
    assert(!work_page_uses_low_refresh_idle(kWorkPageAggregateClock));
    ...
    const WorkPageMask all_pages = static_cast<WorkPageMask>((1U << kWorkPageCount) - 1U);
    assert(is_work_page_enabled(kWorkPageCodexUsage));
    work_page_enabled_mask_store(static_cast<WorkPageMask>(all_pages & ~page_bit(kWorkPageCodexUsage)));
    assert(!is_work_page_enabled(kWorkPageCodexUsage));
    ...
    assert(enabled_data.air_quality);
