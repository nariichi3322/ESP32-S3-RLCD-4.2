...
esp_err_t g_get_u16_result = ESP_ERR_NVS_NOT_FOUND;
uint16_t g_stored_u16 = 0;
int g_get_u16_calls = 0;

esp_err_t g_set_u16_result = ESP_OK;
int g_set_u16_calls = 0;
...
void reset_state()
{
    ...
    g_set_u8_calls = 0;
    g_get_u16_result = ESP_ERR_NVS_NOT_FOUND;
    g_stored_u16 = 0;
    g_get_u16_calls = 0;
    g_set_u16_result = ESP_OK;
    g_set_u16_calls = 0;
}
} // namespace
...
esp_err_t nvs_get_u16(nvs_handle_t, const char *, uint16_t *value)
{
    ++g_get_u16_calls;
    if (g_get_u16_result == ESP_OK && value) {
        *value = g_stored_u16;
    }
    return g_get_u16_result;
}

esp_err_t nvs_set_u16(nvs_handle_t, const char *, uint16_t)
{
    ++g_set_u16_calls;
    return g_set_u16_result;
}
...
int main()
{
    ...
    changed = true;
    assert(network_config_nvs::write_changed_nvs_u16(1, ESP_FAIL, "mask", 0x1ff, &changed) ==
           ESP_FAIL);
    assert(!changed);
    assert(g_get_u16_calls == 0);
    assert(g_set_u16_calls == 0);

    g_get_u16_result = ESP_OK;
    g_stored_u16 = 0x1ff;
    changed = true;
    assert(network_config_nvs::write_changed_nvs_u16(1, ESP_OK, "mask", 0x1ff, &changed) ==
           ESP_OK);
    assert(!changed);
    assert(g_set_u16_calls == 0);

    g_stored_u16 = 0x101;
    g_set_u16_result = ESP_FAIL;
    changed = true;
    assert(network_config_nvs::write_changed_nvs_u16(1, ESP_OK, "mask", 0x1ff, &changed) ==
           ESP_FAIL);
    assert(!changed);
    assert(g_set_u16_calls == 1);

    g_set_u16_result = ESP_OK;
    assert(network_config_nvs::write_changed_nvs_u16(1, ESP_OK, "mask", 0x1ff, &changed) ==
           ESP_OK);
    assert(changed);
    assert(g_set_u16_calls == 2);
    return 0;
}
