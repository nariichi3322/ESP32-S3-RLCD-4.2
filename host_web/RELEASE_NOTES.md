...
- 支持主/备用 Wi-Fi、选填天气城市及离线固定时间；天气城市使用设备当前的 Open-Meteo 流程，留空时恢复 IP 自动定位。
- 主/备用 Wi-Fi 密码默认显示，可通过眼睛按钮独立隐藏。修改字段会使旧链接失效，避免复制过期配置。
...
- 链接包含明文 Wi-Fi 密码，URL 编码不是加密。配置信息仅在本地处理，不上传服务器、不由应用持久化；浏览器历史、剪贴板或分享记录可能留存，请妥善保管，勿公开分享。
