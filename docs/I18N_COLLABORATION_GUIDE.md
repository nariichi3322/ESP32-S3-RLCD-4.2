...
| `Japanese` | Japanese（日本語） | 否 | 使用日文字型資產 |

主要 source files：

- `RLCD_CLOCK/main/ui/ui_language.h`：公開 language API 與 `UiLanguage`。
- `RLCD_CLOCK/main/ui/ui_language.cpp`：目前 language state、locale catalog mapping 與 revision。
- `RLCD_CLOCK/main/ui/ui_i18n.h/.cpp`：以 stable `UiTextId` 管理 weather、weekday、AQI、wind 等共用文字。
- `RLCD_CLOCK/main/ui/ui_widgets.h/.cpp`：共用 LVGL label 建立、更新與 CJK font fallback。
- `RLCD_CLOCK/main/assets/ui_fonts.h`：LVGL font declarations。
- `RLCD_CLOCK/main/storage/device_settings_persistence.cpp`、`RLCD_CLOCK/main/storage/saved_config_loader.cpp`：language 的 NVS persistence 與 startup restore。
- `RLCD_CLOCK/main/ui/ui_views.cpp`：偵測 language revision 並重建一般 firmware UI。

新的翻譯不應以「某個頁面恰好使用的中文 spelling」作為唯一 identity。新共用文字優先使用
`UiTextId`；頁面專用或含參數的文字使用明確的四語 `ui_language_text()`。
...
新增 `UiTextId` 時，必須同時在 `RLCD_CLOCK/main/ui/ui_i18n.cpp` 的 catalog 加入同位置的
Traditional、Simplified、English、Japanese 四語列。檔案中的 `static_assert` 會檢查 enum 與 table 的數量；
不要只新增 enum 或只新增 table row。

### 2.2 四語 inline text：頁面專用或含 format placeholder 的文字
...
- buffer size 必須以四種語言中最長的實際輸出估算，包含數字、單位與終止 `NUL`。

新 code 不應省略第四個 `japanese` 參數。所有四語結果都應由同一份 catalog 提供，
避免不同 call site 遺漏翻譯或產生不一致命名。

### 2.3 `ui_language_localize()`：legacy compatibility only

`ui_language_localize()` 只會處理 generated catalog 中的 exact Traditional/Simplified/English/Japanese literal，
它不是一般用途的 translator：

- 不會翻譯 arbitrary API text、city name、SSID 或使用者輸入。
- dynamic formatted text 通常不會 exact match，因此不能依賴它翻譯 format output。
- 新增 user-visible text 時，請直接使用 `UiTextId` 或四語 `ui_language_text()`。
- 修改既有 catalog literal 時，必須同步檢查 generated catalog 與 locale 行為，並補 test。
...
- 若必須直接呼叫 LVGL API 更新 text，先確認該 label 的 font 能涵蓋四語；一般情況應改用
...
4. 不要為了英文縮短 Traditional/Simplified Chinese 或 Japanese；四種語言應各自保持自然且正確。
...
- 若拆出四語常數，使用 `Traditional`、`Simplified`、`English`、`Japanese` 後綴，並在同一個 semantic group
  放置，避免四種語言散落在不同檔案。
...
架構，以下項目目前固定支援 Traditional、Simplified、English、Japanese 四種語言；未來若新增第五種語言，需同步處理：

- `UiLanguage` enum 的既有 numeric values。
- `ui_language_text(traditional, simplified, english, japanese)` 的四個參數。
- `ui_i18n.cpp` 的四語 catalog 與 `kTexts` table。
- `ui_language_localize()` 的四語 literal mapping。
- Settings language selector、font resource、host tests 與 UI verification。

因此，不能只在 catalog 增加一欄，也不能只修改單一語言的 fallback 就宣稱完成新增語言。

### 7.1 先決定 extension design

開始修改前，先建立明確的 design decision：

1. 保留 typed `UiTextId`，將固定四欄的 text provider 改為依 `UiLanguage` index 查表；或
...
3. 決定 API 如何處理 page-local text 與 format string，避免新增第五、第六個 positional argument
   讓 call site 越來越脆弱。
4. 評估 table、font 與 runtime state 對 Flash、RAM、binary size、partition space 的影響。不要
   因為方便而把整套大型 font 無限制加入 firmware。

這個 design decision 應在 PR 中記錄，並說明是否維持既有四語 call site 的 source compatibility。

### 7.2 實作與 backward compatibility checklist

新增語言時，依下列順序處理：

1. 為既有 `Traditional = 0`、`Simplified = 1`、`English = 2`、`Japanese = 3` 保留 NVS numeric values；不可
...
4. 將共用 translation resource、page-local text、format string、weather/weekday/AQI/wind
   lookup 全部遷移到新的 provider；不可遺留只支援舊四語的 hidden path。
...
2. 決定使用 stable ID、四語 inline text 或 legacy compatibility；新 code 優先前兩者。
3. 同時撰寫 Traditional、Simplified、English、Japanese；確認四者語意、placeholder 與 units 一致。
4. 檢查所有使用該 text 的 label font 與 LVGL long mode。
5. 檢查動態更新、language revision rebuild、startup restore 與 cache 是否會留下舊語言。
6. 為 lookup/format helper 增加 host test；至少覆蓋四種語言、正常值、未知值與邊界值。
7. 執行 targeted compile/static checks；UI 修改應提供四語畫面或硬體驗證證據。
...
- `RLCD_CLOCK/tests/ui_language_host_test.cpp`
- `RLCD_CLOCK/tests/ui_i18n_host_test.cpp`
- `RLCD_CLOCK/tests/ui_weather_board_text_host_test.cpp`
- `RLCD_CLOCK/tests/ui_settings_content_host_test.cpp`
- `RLCD_CLOCK/tests/ui_work_page_catalog_host_test.cpp`

實機或 simulator review 應逐一確認：

- Traditional、Simplified、English、Japanese 四種 language。
...
- [ ] 所有新的 user-visible text 都有 Traditional、Simplified、English、Japanese。
